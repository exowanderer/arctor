...
# More information about the algorithm can be found in the [celerite
# docs](https://celerite.readthedocs.io/) and in the papers ([Paper
# 1](https://arxiv.org/abs/1703.09710) and [Paper
# 2](https://arxiv.org/abs/1801.10156)), but this tutorial will give a
...
with pm.Model() as model:
    ...
    gp = GP(kernel, t, yerr ** 2, model=mean)
...
# There are strong covariances between the parameters so we'll use the
# custom :func:`exoplanet.get_dense_nuts_step` to fit for these
...
# We could have saved these predictions using a `pymc3.Deterministic`
# distribution, but that adds some overhead to each evaluation of the
# model so instead, we can use
# :func:`exoplanet.utils.get_samples_from_trace` to loop over a few random
# samples from the chain and then the :func:`exoplanet.eval_in_model`
# function to evaluate the prediction just for those samples.
