...
class HSTUVISTimeSeries(object):
    ...
    def center_trace(self, stddev=2, plot_verbose=False):
        ...
        for kimg, image in tqdm(enumerate(self.image_stack)):
            ...
            gaussian_fits_ = pool.starmap(self.center_one_tace, zipper)
            ...
            for kcol, results, fitter in gaussian_fits_:
                self.gaussian_fits[kimg][kcol] = {}
                self.gaussian_fits[k]['results'] = results
                self.gaussian_fits[k]['fitter'] = fitter
    ...
    def fit_trace_slope(self, image, stddev=2, plot_verbose=False):
        ...
        if not hasattr(self, 'gaussian_fits'):
            self.center_trace(stddev=stddev, plot_verbose=plot_verbose)

    def plot_trace_peaks(self, image):
        gauss_means = np.zeros(image_shape[1])
        for key, val in self.gaussian_fits.items():
